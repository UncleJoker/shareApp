...
@interface GuiBSetViewController ()<YAWaveViewDelegate,UITableViewDelegate,UITableViewDataSource,HeadViewDelegate,MFMailComposeViewControllerDelegate>

@property (nonatomic, strong) UITableView *list;

@property (nonatomic, strong) GuiBSetTableHeadView *tableHead;
...
@implementation GuiBSetViewController
{
...
- (void)getData{
    ...
    for (int i = 0; i < tempArr.count; i ++) {
        GuiBAddTrainModel *item = tempArr[i];
        if (item.isFinish) {
...
- (void)tapHeadView{
    // 修改用户资料
    GuiBEditUserInfoViewController *userInfo = [GuiBEditUserInfoViewController new];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    GuiBSetTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"setTableCell" forIndexPath:indexPath];
...
- (void)tapCellWith:(NSInteger)index{
    switch (index) {
        case 0:
        {
            GuiBFeedbackViewController *feedbackVC = [GuiBFeedbackViewController new];
        ...
        case 3:
        {
            GuiBAboutUSViewController *aboutVC = [GuiBAboutUSViewController new];
...
- (UITableView *)list{
    if (!_list) {
        ...
        [_list registerClass:[GuiBSetTableViewCell class] forCellReuseIdentifier:@"setTableCell"];
        [_list setTableFooterView:[UIView new]];
    }
    return _list;
}

- (GuiBSetTableHeadView *)tableHead{
    if (!_tableHead) {
        _tableHead = [GuiBSetTableHeadView new];
